...
const Dog_Accessory_List = () => {
  ...
  const Fetch_Dog_Accessories = async () => {
    try {
      const response = await axios.get(`${url}/dog_accessories_list`);
      ...
      toast.error('Dog Accessories list error')
    }
  }

  const Remove_Dog_Accessories = async (id) => {
    const Confirmation = window.confirm('Are you sure you want to delete this dog accessory ?');
    if (Confirmation) {
      try {
        const response = await axios.post(`${url}/dog_accessories_list/remove`, { id });
        if (response.data.success) {
          toast.success(response.data.message);
          await Fetch_Dog_Accessories();
        }
      } catch (error) {
        toast.error('Error occurred while removing the dog accessories');
      }
    } else {
  ...
  const handleUpdateProduct = async (e) => {
    ...
    try {
      const response = await axios.put(`${url}/dog_accessories_update/${selectedProduct._id}`, updatedProduct);
      if (response.data.success) {
        toast.success(response.data.message || 'Dog Accessory updated successfully');
        document.getElementById('update_modal').close(); 
        Fetch_Dog_Accessories();
      } else {
        toast.error(response.data.message || 'Failed to update dog accessory');
      }
    } catch (error) {
      toast.error('Error occurred while updating the dog accessory');
    }
  };
  ...
  return (
    <>
      <p className='mb-5 font-medium'>Dog Accessories List</p>
              ...
              <tbody key={index}>
                <tr className='border border-indigo-500'>
                  ...
                  <td className="py-3 px-5 text-start">
                    <i className="fa-solid fa-xmark hover:text-red-600 text-lg cursor-pointer" onClick={() => Remove_Dog_Accessories(product._id)} ></i>
                  </td>
                </tr>
              </tbody>
            )) 
              ...
              <table className='flex'>
                <thead>
                  <tr className='flex flex-col'>
                    ...
                    <th className="py-3 text-start lg:px-3 px-5 text-sm">Type</th>
                    <th className="py-3 text-start lg:px-3 px-5 text-sm">Price</th>
                    <th className="py-3 text-start lg:px-3 px-5 text-sm">Description</th>
                  </tr>
                </thead>
                <tbody>
                  <tr className='flex flex-col'>
                    <td className="py-3 text-start lg:px-3 px-5 text-sm">{selectedProduct.name}</td>
                    <td className="py-3 text-start lg:px-3 px-5 text-sm">{selectedProduct.category}</td>
                    <td className="py-3 text-start lg:px-3 px-5 text-sm">{selectedProduct.type}</td>
                    <td className="py-3 text-start lg:px-3 px-5 text-sm">{selectedProduct.price}</td>
                    <td className="py-3 text-start lg:px-3 px-5 text-sm">{selectedProduct.desc}</td>
                  </tr>
                </tbody>
              </table>
            </div>
      ...
      <dialog id="update_modal" className="modal w-full lg:w-fit md:w-fit">
        <div className="modal-box p-6 rounded-lg shadow-lg bg-white">
          <form onSubmit={handleUpdateProduct}>
            ...
            <button type="submit" className="mt-8 p-3 w-[max(40vw,200px)] text-lg text-white bg-gradient-to-br from-indigo-500 to-purple-500 shadow-xl hover:bg-gradient-to-l">
              Update
            </button>
          </form>
        </div>
...
export default Dog_Accessory_List
