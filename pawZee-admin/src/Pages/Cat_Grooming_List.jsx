...
const Cat_Grooming_List = () => {
  ...
  const Fetch_Cat_Grooming = async () => {
    try {
      const response = await axios.get(`${url}/cat_groomings_list`);
      ...
      toast.error('Cat grooming list error')
    }
  }

  const Remove_Cat_Grooming = async (id) => {
    const Confirmation = window.confirm('Are you sure you want to delete this cat grooming product ?');
    if (Confirmation) {
      try {
        const response = await axios.post(`${url}/cat_groomings_list/remove`, { id });
        if (response.data.success) {
          toast.success(response.data.message);
          await Fetch_Cat_Grooming();
        }
      } catch (error) {
  ...
  const handleUpdateProduct = async (e) => {
    ...
    try {
      const response = await axios.put(`${url}/cat_groomings_update/${selectedProduct._id}`, updatedProduct);
      if (response.data.success) {
        toast.success(response.data.message || 'Cat Grooming updated successfully');
        document.getElementById('update_modal').close(); 
        Fetch_Cat_Grooming()
      } else {
        toast.error(response.data.message || 'Failed to update cat grooming');
      }
    } catch (error) {
      toast.error('Error occurred while updating the cat grooming');
    }
  };
  ...
  return (
    <>
      <p className='mb-5 font-medium'>Cat Grooming List</p>
          ...
          { data && data.length > 0 ? (
              ...
              <tbody key={index}>
                <tr className='border border-indigo-500'>
                  ...
                  <td className="py-3 px-5 text-start">
                    <i className="fa-solid fa-xmark hover:text-red-600 text-lg cursor-pointer" onClick={() => Remove_Cat_Grooming(product._id)} ></i>
                  </td>
                </tr>
              </tbody>
            )) 
            ...
            <tr className='text-center'>
              <td colSpan="10" className="py-3">
                No Cat Grooming Products Found
              </td>
            </tr>
          )}
...
export default Cat_Grooming_List
