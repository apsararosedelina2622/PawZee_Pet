...
const Dog_Food_List = () => {
  ...
  const Fetch_Dog_Foods = async () => {
    try {
      ...
      toast.error('Dog food list error')
    }
  }

  const Remove_Dog_Foods = async (id) => {
    const Confirmation = window.confirm('Are you sure you want to delete this dog food ?');
    if (Confirmation) {
      try {
        const response = await axios.post(`${url}/dog_foods_list/remove`, { id });
        if (response.data.success) {
          toast.success(response.data.message);
          await Fetch_Dog_Foods();
        }
      } catch (error) {
  ...
  const handleUpdateProduct = async (e) => {
    ...
    try {
      const response = await axios.put(`${url}/dog_foods_update/${selectedProduct._id}`, updatedProduct);
      if (response.data.success) {
        toast.success(response.data.message || 'Dog Food updated successfully');
        document.getElementById('update_modal').close(); 
        Fetch_Dog_Foods();
      } else {
        toast.error(response.data.message || 'Failed to update dog food');
      }
    } catch (error) {
      toast.error('Error occurred while updating the dog food');
    }
  };
  ...
  return (
    <>
      <p className='mb-5 font-medium'>Dog Food List</p>
            ...
            data.map((product, index) => (
              <tbody key={index}>
                <tr className='border border-indigo-500'>
                  ...
                  <td className="py-3 lg:px-3 px-5">
                    <button className='text-white py-1 px-3 rounded-lg bg-gradient-to-br from-[#6a70d1] to-purple-400' 
                  ...
                  <td className="py-3 px-5 text-start">
                    <i className="fa-solid fa-xmark hover:text-red-600 text-lg cursor-pointer" onClick={() => Remove_Dog_Foods(product._id)} ></i>
                  </td>
                </tr>
              </tbody>
            ))
        ...
        <div className="modal-box p-6 rounded-lg shadow-lg bg-white">
          ...
          {selectedProduct && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <img src={selectedProduct.image} alt="Product" className="h-80 w-full object-cover rounded-md scale-y-110" />
              </div>
      ...
      <dialog id="update_modal" className="modal w-full lg:w-fit md:w-fit">
        <div className="modal-box p-6 rounded-lg shadow-lg bg-white">
          <form onSubmit={handleUpdateProduct}>
            ...
            <button type="submit" className="mt-8 p-3 w-[max(40vw,200px)] text-lg text-white bg-gradient-to-br from-[#6a70d1] to-purple-400 shadow-xl hover:bg-gradient-to-l">
              Update
            </button>
          </form>
        </div>
