...
const Cat_Food_List = () => {
  ...
  const Fetch_Cat_Foods = async () => {
    try {
      ...
      toast.error('Cat food list error')
    }
  }

  const Remove_Cat_Foods = async (id) => {
    const Confirmation = window.confirm('Are you sure you want to delete this cat food ?');
    if (Confirmation) {
      try {
        const response = await axios.post(`${url}/cat_foods_list/remove`, { id });
        if (response.data.success) {
          toast.success(response.data.message);
          await Fetch_Cat_Foods();
        }
      } catch (error) {
        toast.error('Error occurred while removing the cat food');
      }
    } else {
  ...
  const handleUpdateProduct = async (e) => {
    ...
    try {
      const response = await axios.put(`${url}/cat_foods_update/${selectedProduct._id}`, updatedProduct);
      if (response.data.success) {
        toast.success(response.data.message || 'Cat Foods updated successfully');
        document.getElementById('update_modal').close(); 
        Fetch_Cat_Foods();
      } else {
        toast.error(response.data.message || 'Failed to update cat foods');
      }
    } catch (error) {
      toast.error('Error occurred while updating the cat foods');
    }
  };
  ...
  return (
    <>
      <p className='mb-5 font-medium'>Cat Food List</p>
              ...
              <tbody key={index}>
                <tr className='border border-indigo-500'>
                  ...
                  <td className="py-3 px-5 text-start">
                    <i className="fa-solid fa-xmark hover:text-red-600 text-lg cursor-pointer" onClick={() => Remove_Cat_Foods(product._id)} ></i>
                  </td>
                </tr>
              </tbody>
            )) 
        ...
        <div className="modal-box p-6 rounded-lg shadow-lg bg-white">
          ...
          {selectedProduct && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <img src={selectedProduct.image} alt="Product" className="h-80 w-full rounded-md scale-y-110" />
              </div>
