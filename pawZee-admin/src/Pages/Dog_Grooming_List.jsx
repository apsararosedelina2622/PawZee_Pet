...
const Dog_Grooming_List = () => {
  ...
  const Fetch_Dog_Grooming = async () => {
    try {
      const response = await axios.get(`${url}/dog_groomings_list`);
      ...
      toast.error('Dog grooming list error')
    }
  }

  const Remove_Dog_Grooming = async (id) => {
    const Confirmation = window.confirm('Are you sure you want to delete this dog grooming product ?');
    if (Confirmation) {
      try {
        const response = await axios.post(`${url}/dog_groomings_list/remove`, { id });
        if (response.data.success) {
          toast.success(response.data.message);
          await Fetch_Dog_Grooming();
        }
      } catch (error) {
        toast.error('Error occurred while removing the dog food');
      }
    } else {
  ...
  const handleUpdateProduct = async (e) => {
    ...
    try {
      const response = await axios.put(`${url}/dog_groomings_update/${selectedProduct._id}`, updatedProduct);
      if (response.data.success) {
        toast.success(response.data.message || 'Dog Grooming Product updated successfully');
        document.getElementById('update_modal').close(); 
        Fetch_Dog_Grooming();
      } else {
        toast.error(response.data.message || 'Failed to update dog grooming');
      }
    } catch (error) {
      toast.error('Error occurred while updating the dog grooming');
    }
  };
  ...
  return (
    <>
      <p className='mb-5 font-medium'>Dog Grooming Products List</p>
          ...
          { data && data.length > 0 ? (
              ...
              <tbody key={index}>
                <tr className='border border-indigo-500'>
                  ...
                  <td className="py-3 px-5 text-start">
                    <i className="fa-solid fa-xmark hover:text-red-600 text-lg cursor-pointer" onClick={() => Remove_Dog_Grooming(product._id)} ></i>
                  </td>
                </tr>
              </tbody>
            )) 
            ...
            <tr className='text-center'>
              <td colSpan="10" className="py-3">
                No Dog Grooming Products Found
              </td>
            </tr>
          )}
        </table>
...
export default Dog_Grooming_List
