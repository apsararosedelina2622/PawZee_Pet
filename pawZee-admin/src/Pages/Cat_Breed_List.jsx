...
const Cat_Breed_List = () => {
  ...
  const Fetch_Cat_Breeds = async () => {
    try {
      ...
      toast.error('Cat breed list error');
    }
  };

  const Remove_Cat_Breeds = async (id) => {
    const Confirmation = window.confirm('Are you sure you want to delete this cat breed ?');
    if (Confirmation) {
      try {
        const response = await axios.post(`${url}/cat_breeds_list/remove`, { id });
        if (response.data.success) {
          toast.success(response.data.message);
          await Fetch_Cat_Breeds();
        }
      } catch (error) {
        toast.error('Error occurred while removing the cat breed');
      }
    } else {
  ...
  const handleUpdateProduct = async (e) => {
    ...
    try {
      const response = await axios.put(`${url}/cat_breeds_update/${selectedProduct._id}`, updatedProduct);
      if (response.data.success) {
        toast.success(response.data.message || 'Cat Breed updated successfully');
        document.getElementById('update_modal').close(); 
        Fetch_Cat_Breeds();
      } else {
        toast.error(response.data.message || 'Failed to update cat breed');
      }
    } catch (error) {
      toast.error('Error occurred while updating the cat breed');
    }
  };
  ...
  return (
    <>
      <p className='mb-5 font-medium'>Cat Breed List</p>
              ...
              <tbody key={index}>
                <tr className='border border-indigo-500'>
                  ...
                  <td className="py-3 px-5 text-start">
                    <i className="fa-solid fa-xmark hover:text-red-600 text-lg cursor-pointer" onClick={() => Remove_Cat_Breeds(product._id)} ></i>
                  </td>
                </tr>
              </tbody>
            ))
