...
const Cat_Accessory_List = () => {
  ...
  const Fetch_Cat_Accessories = async () => {
    try {
      const response = await axios.get(`${url}/cat_accessories_list`);
      if (response.data.success) {
        setData(response.data.pet)
      }
    } 
    catch (error) {
      toast.error('Cat Accessories list error')
    }
  }

  const Remove_Cat_Accessories = async (id) => {
    const Confirmation = window.confirm('Are you sure you want to delete this cat accessory ?');
    if (Confirmation) {
      try {
        const response = await axios.post(`${url}/cat_accessories_list/remove`, { id });
        if (response.data.success) {
          toast.success(response.data.message);
          await Fetch_Cat_Accessories();
        }
      } catch (error) {
        toast.error('Error occurred while removing the cat accessories');
      }
    } else {
  ...
  const handleUpdateProduct = async (e) => {
    ...
    try {
      const response = await axios.put(`${url}/cat_accessories_update/${selectedProduct._id}`, updatedProduct);
      if (response.data.success) {
        toast.success(response.data.message || 'Cat Accessory updated successfully');
        document.getElementById('update_modal').close(); 
        Fetch_Cat_Accessories();
      } else {
        toast.error(response.data.message || 'Failed to update cat Accessory');
      }
    } catch (error) {
      console.log(error);
      toast.error('Error occurred while updating the Cat Accessory');
    }
  };
  ...
  return (
    <>
      <p className='mb-5 font-medium'>Cat Accessories List</p>
              ...
              data.map((product, index) => (
                <tbody key={index}>
                  <tr className="border-b border-indigo-500">
                    ...
                    <td className="py-3 lg:px-3 px-5">
                      <i className="fa-solid fa-eye text-purple-600 cursor-pointer" onClick={() => handleViewProduct(product)}></i>
                    </td>
                    <td className="py-3 lg:px-3 px-5">
                      <button className='text-white py-1 px-3 rounded-lg bg-gradient-to-br from-[#6a70d1] to-purple-400 to-purple-400' onClick={() => handleUpdateModal(product)}>
                        Update
                      </button>
                    </td>
                    <td className="py-3 px-5 text-start">
                      <i onClick={() => Remove_Cat_Accessories(product._id)} className="fa-solid fa-xmark hover:text-red-600 text-lg cursor-pointer"></i>
                    </td>
                  </tr>
                </tbody>
              ))
...
export default Cat_Accessory_List
